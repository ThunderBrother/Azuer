...
- (void)setTitle:(NSString *)title
    ...
    if (clickBlock) {
        ...
        UITapGestureRecognizer *lbTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(subTitleLbTap:)];
        self.subtitleLb.userInteractionEnabled = YES;
        [self.subtitleLb addGestureRecognizer:lbTap];
    }
}

- (void)setSubTitleColor:(UIColor *)color{
    self.subtitleLb.textColor = color;
}
...
- (void)subTitleLbTap:(UITapGestureRecognizer *)tap{
    if (self.clickBlock) {
        self.clickBlock(kOTSExceptionBlockSubTitleType);
    }
}
...
- (UILabel *)titleLb{
    if (_titleLb == nil) {
        ...
        _titleLb.preferredMaxLayoutWidth = self.frame.size.width - 20;
    }
    return _titleLb;
}
- (UILabel *)subtitleLb{
