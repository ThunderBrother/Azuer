...
+ (CGSize)sizeForCellData:(id)aData atIndexPath:(NSIndexPath *)indexPath {
    return CGSizeMake(UI_CURRENT_SCREEN_WIDTH, [self heightForCellData:aData atIndexPath:indexPath]);
}
...
- (BOOL)becomeFirstResponder {
    UIView *contentView = self.contentView.subviews.firstObject;
    if ([contentView conformsToProtocol:@protocol(OTSDynamicContentViewProtocol)]) {
