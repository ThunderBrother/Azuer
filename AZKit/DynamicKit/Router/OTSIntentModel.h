...
FOUNDATION_EXTERN NSString const* OTSIntentImageNameKey;
...
typedef NS_OPTIONS(NSUInteger, OTSActionOption) {
    ...
    OTSActionOptionShowEmpty = 1 << 16,
    OTSActionOptionHideEmpty = 1 << 17
...
typedef NS_OPTIONS(NSUInteger, OTSRouterOption) {
    ...
    OTSRouterPushOptionClearTop    = 1 << 4,   //if option & OTSRouterPush, push item and clear items before
    OTSRouterPushOptionSingleTop   = 1 << 5,   //if option & OTSRouterPush, remove all item.class in stack before pushing it.
    OTSRouterPushOptionRootTop     = 1 << 6,   //if option & OTSRouterPush, push item and remove items to make sure that there are less equal than two vcs in stack
    OTSRouterPushOptionClearLast   = 1 << 7,   //if option & OTSRouterPush, push item and remove the last one
    OTSRouterPresentOptionWrapNC   = 1 << 8,   //if option & OTSRouterPresent, wrap destination with UINavigationController
    
    OTSRouterModalOptionBlur                = 1 << 11,  //if option & OTSRouterModal, add a dark blur background, default is a alpha-dark background when on modal window and transparent on top window
    OTSRouterModalOptionOnTopWindow         = 1 << 14, //if option & OTSRouterModal, content vc will be on the top window, default is on the modal window
    OTSRouterModalOptionContentBottom       = 1 << 12, //if option & OTSRouterModal, content view will be placed at bottom, default is centered
    OTSRouterModalOptionContentTop          = 1 << 13, //if option & OTSRouterModal, content view will be placed at top
