...
NSString *const OTSSystemErrorString = @"Oops, something unexpected happened!";
...
+ (instancetype)managerWithOwner:(id)owner {
    //OTSNetworkLog core data manager构造会阻塞主线程，所以让它先初始化
    //[OTSNetworkLog sharedInstance];
...
- (void)dealloc
{
    OTSLogW(@"[%@ call %@ --> %@]", [self class], NSStringFromSelector(_cmd), _hostClassName);
}
...
- (NSURLSessionDataTask *)requestWithParam:(OTSOperationParam *)aParam {
    ...
    if (aParam.requestType == kRequestPost) {//POST方式
        requestOperation = [self POST:requestUrl parameters:params progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            STRONG_SELF;
        ...
        requestOperation = [self GET:requestUrl parameters:params progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            STRONG_SELF;
            [self successWithTask:task responseObject:responseObject param:aParam];
            OTSLogF(@"(%lld)接口返回成功\n接口名%@\n接口内容%@\n\n", (long long)([[OTSGlobalValue sharedInstance].serverTime timeIntervalSince1970]*1000), aParam.requestUrl, responseObject);
        } failure:^(NSURLSessionDataTask *task, NSError *error) {
                  ...
                  param:(OTSOperationParam *)aParam
{
    ...
    if (runCallBack) {
        ...
        if (rtnCode.length>0 && ![rtnCode isEqualToString:@"0"]) {
            ...
            if ([rtnCode isEqualToString:@"000000000007"]) {
                msg = OTSSystemErrorString;
            } else {
                msg = msg ?: OTSSystemErrorString;
            }
            
            NSMutableDictionary *userInfo = [responseObject mutableCopy];
            userInfo[NSLocalizedDescriptionKey] = msg;
            userInfo[@"rtn_msg"] = msg;
...
- (NSString *)getAesEncodedToken
{
    ...
    NSAssert([OTSNetworkManager sharedInstance].encryptByAESKey, @"Don't you forget set encryptByAESKey for OTSNetworkManager?");
    NSString *encodedStr = [OTSNetworkManager sharedInstance].encryptByAESKey(totalStr, signatureKey);
    return encodedStr;
}
...
- (void)cancelOperationsAndRemoveFromNetworkManager
{
    ...
    [[OTSNetworkManager sharedInstance] removeOperationManager:self];
}
