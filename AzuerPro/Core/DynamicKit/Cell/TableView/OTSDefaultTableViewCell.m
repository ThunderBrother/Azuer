...
#import "OTSDefaultTableViewCell.h"
#import "OTSTableViewItem.h"

#define OTSDefaultTableViewCellInnerMargin 4.0

@implementation OTSDefaultTableViewCell
...
+ (CGFloat)heightForCellData:(id)aData atIndexPath:(NSIndexPath *)indexPath {
    ...
    OTSTableViewItem *item = (OTSTableViewItem*)aData;
    if (!item.subTitleAttribute.title.length) {
        return originalHeight;
    }
    ...
    if (subTitleHeight) {
        resultHeight += (OTSDefaultTableViewCellInnerMargin + subTitleHeight);
    }
    return resultHeight;
}
...
- (void)__layoutCellSubViews {
    ...
    if (_rightTitleLabel.text) {
        ...
        if (_rightSubTitleLabel.text) {
            ...
            _rightSubTitleLabel.top = height * .5f + OTSDefaultTableViewCellInnerMargin * .5f;
            
            _rightTitleLabel.right = edgeRight;
            _rightTitleLabel.bottom = height * .5f - OTSDefaultTableViewCellInnerMargin * .5f;
    ...
    if (_leftTitleLabel.text) {
        ...
        if (_leftSubTitleLabel.text) {
            ...
            _leftSubTitleLabel.top = _leftTitleLabel.bottom + OTSDefaultTableViewCellInnerMargin;
